...
/// The render state
renderer_state: _renderer.State,

/// The renderer thread manager
renderer_thread: _renderer.Thread,
...
const DerivedConfig = struct {
    ...
    fn scaledPadding(self: *const DerivedConfig, x_dpi: f32, y_dpi: f32) _renderer.Padding {
...
pub fn init(
    ...
    const size: _renderer.Size = size: {
        ...
        const explicit: _renderer.Padding = derived_config.scaledPadding(
            x_dpi,
            y_dpi,
        );
    ...
    var render_thread = try _renderer.Thread.init(
        alloc,
        config,
        rt_surface,
        &self.renderer,
...
fn updateRendererHealth(self: *Surface, health: _renderer.Health) void {
...
pub fn updateConfig(
    ...
    var renderer_message = try _renderer.Message.initChangeConfig(self.alloc, config);
...
fn setCellSize(self: *Surface, size: _renderer.CellSize) !void {
...
pub fn sizeCallback(self: *Surface, size: apprt.SurfaceSize) !void {
    ...
    const new_screen_size: _renderer.ScreenSize = .{
        .width = size.width,
        .height = size.height,
    };
...
fn resize(self: *Surface, size: _renderer.ScreenSize) !void {
...
pub fn preeditCallback(self: *Surface, preedit_: ?[]const u8) !void {
    ...
    const Codepoint = _renderer.State.Preedit.Codepoint;
...
fn mouseReport(
    ...
    switch (self.io.terminal.flags.mouse_format) {
        ...
        .sgr_pixels => {
            ...
            const coord: _renderer.Coordinate.Terminal = (_renderer.Coordinate{
                .surface = .{
                    .x = pos.x,
                    .y = pos.y,
                },
...
pub fn posToViewport(self: Surface, xpos: f64, ypos: f64) terminal.point.Coordinate {
    // Get our grid cell
    const coord: _renderer.Coordinate = .{ .surface = .{ .x = xpos, .y = ypos } };
